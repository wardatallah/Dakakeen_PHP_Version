...
<div class="left_content">
      ...
      <ul class="left_menu">
				...
				<!--<li  class='odd'><a href='user.php?menu=Sales' style='background: url(images/add-to-cart1.png) no-repeat left #f1f1f1;'>My Sales</a></li>-->
...
$sql = "SELECT good_Id,good_name,price,img_path,com_id FROM goods where com_id=$id";
...
//create a query
$sql = "SELECT sender,send_date,ID,done FROM msgs where rec_id=$id order by ID desc";
...
<li class='items'>
<div class='product-box spacer' style='height: 13%;width:80%;"; 
if ($done==1)
...
//create a query
$sql = "SELECT company_name,address,category,phone,email,password FROM users where user_name='$nameuser'";
...
<li class='items'>
		...
		<form id='contacts-form' method=post action='change_settings.php' enctype='multipart/form-data'>
			...
			<div class='field' >
				<label for=''>Address : </label>
			    <input type='text' name='address' value=\"$row[1]\"/>
			</div>
			<div class='field' >
				<label for=''>Category : </label>
			    <input type='text' name='categ' value=\"$row[2]\"/>
			</div>
			<div class='field' >
				<label for=''>Phone : </label>
